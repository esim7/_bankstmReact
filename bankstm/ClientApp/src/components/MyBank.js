...
export class MyBank extends Component {
                        ...
                        <div className="col-md-8">
                            <div className="card-body">
                                ...
                                <h3>Срок действия до: {data.validPeriod.slice(0, -17)} </h3>
                                    <h4>Остаток денежных средств на счету: {totalSum} <i className="fas fa-tenge"></i></h4>
    
                            </div>
                            <span> 
                                    <Link tag={Link} onClick={() => onGetCurrentAccountData(data.id)} className="btn btn-outline-info float-right mr-5 mb-2" to="/account-details">Детали
    ...
    async getData() {
        ...
        var total = Array.from(data[0].bankCards).reduce((res, i) => res + i.amount, 0);
        this.setState({ accountData: data, totalSum: total});
